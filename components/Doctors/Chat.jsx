        ...
        <div className=' w-screen h-screen flex flex-col'>
            {
                section == "chat" ?
                    <>
                        <div className=' w-screen h-screen bg-[url("/images/chat.jpg")] bg-cover flex flex-col'>
                            <div className='w-full h-12 bg-[#158a7f] flex items-center justify-between  px-4 '>
                                <div className='flex items-center'>
                                    <Image src={"/images/p1.svg"} width={100} height={100} className='w-10 h-10' />
                                    <span className='text-white ms-2 '>Dr. Marcus Horizon</span>
                                </div>
                                <div className='flex me-4'>
                                    {/* <Link href={"/doctors/adsf/call"} > */}
                    ...
                    <span className='px-2 py-1  text-white rounded-t-lg rounded-bl-lg'>hi there</span>
                </div> */}
                            </div>
                            <form onSubmit={SendMsg} className='w-full h-16 bg-none p-4 flex'>
                                <input
                                    value={inputValue}
                                    onChange={(e) => setInputValue(e.target.value)}
                                    ref={inputRef} // Set the reference to the input
                                    type="text" name='msg' className=' rounded-full h-10 ps-4 bg-[#219C90] text-sm outline-none placeholder:text-gray-200 text-white ' placeholder='write message...' />
                                <button type='submit' className='w-10 h-10 rounded-full bg-green-400 ms-6 text-white text-2xl  flex justify-center items-center'><i class="ri-send-plane-fill"></i></button>
